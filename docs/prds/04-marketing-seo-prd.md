...
- ~~**Multilingual SEO** — English-only in MVP.~~ **Reversed by the owner on 2026-09-05 (D-123, built as B-262).** The site publishes in English and Spanish, at separate URLs (`/faq` and `/es/faq`), each declaring the other with `hreflang` and each carrying its own canonical. What made this a non-goal was cost, and what changed it was the cost of the alternative: D-122 kept the crawler on English by carrying the locale in a cookie, which meant a Spanish visitor and Googlebot read different words from one URL — the state D-77's duplicate-content gate reasons about — the Spanish prose earned no ranking at all, and a Spanish speaker could not share a Spanish page, because the link they copied rendered in English for whoever opened it. **The non-goal that remains is a third language**: nothing here is scoped for one, and the machinery is deliberately keyed rather than open-ended so that adding one is a typecheck failure in every place that must be touched.
...
- AC1: City pages at `/storage/{state}/{city}` list all facilities in that city with starting price and rating; unique intro copy per city; indexable only when ≥1 facility exists in the city.
  - *Built B-082 part 2, 2026-08-17.* The path is `/storage/…` per **D-32**, the same correction US-1 AC1 above already carries. **Distance is not shown (D-59)** — a city page is reached without anybody naming a location, so there is no origin to measure from; the list is ordered cheapest-first instead, and the page carries the search form, which is where a distance can be honest. **The intro copy is generated from the facilities in the city (D-58) unless somebody has written it (D-62, B-128).** D-58 refused a `City` model outright, on the footing that derived copy cannot drift; B-128 completed it rather than reversing it, because this site's own duplicate-content report scored the generated intros at 0.82–0.85 against each other — over the codebase's own 0.8 threshold — so pages built to rank were duplicate content with no field anybody could edit. A `City` row now holds authored prose (and nothing else, keyed on the URL slug), editable at `/admin/settings/marketing/cities` under a `marketing:city_copy` permission asked for with a null facility; a city nobody has written for still renders the generated intro, which is every city on the day it shipped. "Indexable only when ≥1 facility exists" is enforced as a **404**, not a `noindex` on an empty page, and the sitemap builds its city list from the same function the page does — so it cannot advertise a URL the page refuses to render.
  - *Per-city/size landing pages built B-089, 2026-08-20 (**D-77**).* A sibling page type at `/storage/{state}/{city}/size/{dimension}` — the query "10x10 storage austin" is high-intent and is not the query the facility page targets. Generated from real priced inventory, 404ing where there is none, exactly as AC1's indexability rule works for a city. **The difference from a city page is that this one is gated before publication, not reported on afterwards:** B-082 part 6's detector scores each intro against its sibling sizes' and a page at or over 0.8 is served `noindex, follow` and left out of the sitemap. What lets the standard sizes through is that each carries two sentences about its own size from the shared `UNIT_SIZES` catalogue — measured at 0.738 across the seven, against 0.940 for two sizes the catalogue does not cover, which are correctly gated (**B-134** is where an authored override goes if a real portfolio needs it). D-60 is unweakened: the guide is linked, and a landing page reproduces neither its seven entries nor the comparisons of sizes it is not about.
