...
| D-132 | **How the Neon dev branch gets a truthful migration baseline** (raised 2026-09-10 while the buildable queue was empty; the condition itself is older — **B-272** recorded it as "the 116 the `.env.local` half reports as unapplied", and it began presenting as a hard `P3005` error once **B-274** gave `migrate deploy` a migration it actually wanted to write) |
| D-133 | **Whether the site OFFERS Spanish to a visitor whose browser prefers it** (raised by the digital-experience review, 2026-09-10; **blocks B-290**) |
| D-134 | **Whether `<title>` follows the reader's language while the rest of a page's metadata stays English** (raised by the accessibility review, 2026-09-10; **blocks B-291**; **amends the inline call B-262 made with no D-number**) |
| D-135 | **Whether a keyboard gap closed in the same build is named in the accessibility statement's "Where we fall short" list** (raised while building B-282, 2026-09-10) |
| D-136 | **Which messages a business account's payer receives beside the lease's tenant** (raised while building **B-279**, 2026-09-10; the row's remedy said "`Invoice` events" and its title said "a bill or a past-due notice", and in the code those are not the same set) |
| D-137 | **What the counter does with more money than a tenant owes when their unit is on somebody else's business account** (raised while building **B-280**, 2026-09-10. The row's acceptance said the account's check "taken against an employee's unit no longer banks the remainder as prepayment on that unit". Its remedy, an account option in the picker, does not by itself stop a staffer keying the check to the unit) |
| D-138 | **Whether a report date range is a pair of calendar dates or a pair of instants, when it filters both kinds of column** (raised and settled while building **B-297**, 2026-09-12; **B-296** deferred it as "one range cannot serve both, and sorting out which callers need which is a row, not a clause") |
| D-139 | **What `Lease.startDate` is: an instant, or a facility-local calendar day** (raised and left open by **D-138** — "it is neither kind of column and neither bound is right for all of its rows" — and settled while building **B-298**, 2026-09-12) |
| D-140 | **Whether a Spanish tenant's recapture line is Spanish on the ledger** (D-122's open half, raised in **B-284**'s and **B-290**'s carried-forward gaps; **answered by the owner 2026-09-14**; no build) |
| D-141 | **What the business account's payer is sent, when one invoice event is raised per unit** (raised and settled while building **B-309**, 2026-09-16; **D-136**'s own named gap — that row decided WHICH events reach the payer and deliberately left the shape of them alone) |
