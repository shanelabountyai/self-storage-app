...
- Native app vs. PWA spike for Bluetooth unlock (see OQ-2); recommendation remains **web-first** until this phase forces the question
...
- **OQ-2:** Native app vs. PWA for Phase 3 Bluetooth unlock — Web Bluetooth is not reliably available on iOS Safari, and real vendors (Nokē) push tenants into the vendor's own app; does that make our own native app moot? Needs a spike + vendor-partnership reality check.
...
- **OQ-8:** Camera embeds — some vendor viewers forbid iframing (X-Frame-Options); links-only may be the floor. Verify per chosen camera vendor; keep FR-10's "links or sandboxed iframes" flexible.
