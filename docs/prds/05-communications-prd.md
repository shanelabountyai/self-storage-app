...
- **B-309 amends this for a business account's payer (D-141).** "One-tap way to pay" was literally true and practically false for accounts payable: a fifteen-unit account raised fifteen reminders at the payer, each with its own `mintPayLink` opening a **single-unit** screen, so the one-tap reading was *fifteen taps and fifteen card payments*. The payer now gets **one message per account per business day per event class**, and its link is `/portal/pay?account=<id>` — B-256's account pay screen, which settles every unit at once and itemises them. The lease's own tenant keeps the per-lease magic link unchanged, and a payer who is also that unit's tenant is the tenant, so they do too. `payNowLink` therefore returns the account URL for an added payer before it mints anything, which is also why the fifteen throwaway `PayLink` rows no longer exist.
...
- **AC (added 2026-08-21 from the operator review; needs confirmation at build time): "notice sent" means a notice accepted for delivery, not a status column set before the event fired.** PRD 02 US-11 blocks an effective date that violates the minimum notice period — a guarantee about *delivery* that the code currently makes about *intent*. Before an increase applies, its notice is reconciled against the `Message` record: a hard bounce, a suppression-list hit, or no send record at all is a **blocking** condition raising a high-priority task rather than a quiet application 30 days later. **What then happens to the increase is D-88**, and this AC is written to whichever answer that decision takes. D-37's email-only scope is unchanged — this adds a check, not a channel.
...
**CN-23 [MVP]** **A reservation reminder is scoped by what the hold is for.** *(Added 2026-08-21 from the operator review; verified against `sendExpiringSoonReminders`.)* The expiring-soon sweep selects held reservations with **no filter on `source`**, so a transfer hold (D-82, `source = 'transfer'`) receives `reservation_expiring_soon` — copy telling an existing tenant to "use the link from your original confirmation email to complete your move-in online", a link D-82 deliberately ensured never exists, for a move-in that is not happening. Reusing `Reservation` for the transfer hold inherited its whole mailing list, and that is the consequence.
- AC: every reminder, notice and sweep reading `Reservation` states which `source` values it applies to. A new hold source is a filter to review, never a mailing list joined by default; a test asserts a transfer hold receives no move-in-shaped message.
- AC: a tenant whose transfer hold is expiring **is told when it expires** — an absolute facility-local date and time — and what to do about it, which is to talk to the office rather than click a link they do not have.
- AC: the transfer-request confirmation, the pending-request screen and this reminder state the same expiry from the same helper (`holdWindowSentence`), so the email and the screen cannot disagree.

**CN-24 [P2]** **A payment plan is four messages, and the one that matters is the break.** *(Added 2026-08-25 from the operator, UX and accessibility reviews independently — three reports, one gap; verified by grep: before this, no rule anywhere in the module named a payment plan.)* PRD 02 US-25's `payment_plan` hold halts dunning, late fees and access suspension, and B-090c gave it a schedule — with no message at any point in its life. The whole tenant-facing signal was one portal dashboard card, which **unmounts the moment the plan breaks**, so the hold lifted, the ladder resumed, late fees re-armed and the gate closed with the tenant's first news of any of it being the next dunning letter, or the keypad.
- **Plan agreed** — on creation: every installment with its date and amount, the total, what happens if one is missed, and what the plan does and does not pause. Sent before the first installment is due, not as a receipt afterwards.
- **Installment due** — a per-facility configurable number of days ahead, sent **whether or not autopay will take it** (D-97 decides which). A tenant who thinks a payment is automatic and is wrong loses their plan over a misunderstanding, and the pre-charge notice precedent is already D-11a's.
- **Plan broken** — **the same night the hold lifts** (the breach job runs at hour 4, before the dunning ladder at 5 and the timeline at 6), stating plainly that collections have resumed, what is now owed, and what it costs to put right. This is the message the whole rule exists for; a plan that breaks in silence is worse than no plan, because the tenant stopped watching the balance on the strength of an agreement.
- **Plan kept** — on completion. Cheap, and it is the only message in the set that arrives as good news.
- AC: **transactional, not marketing** — not gated on marketing consent, and not subject to quiet-hours suppression as marketing; SMS still honours SMS consent and `isSmsQuietHours` (FR-MSG-5). Any suite asserting one of these **pins the clock**, per the trap three suites already fell into.
- AC: every send is idempotent per `(plan, installment, event, recipient)` — a catch-up run walking several business dates must not post four break notices for one break, the same invariant FR-4 already carries.
- AC: email meets **FR-9a** as B-084 part 3 established it — `lang` on the root, ordered headings with nothing skipped, `<th scope>` on the schedule, a `<caption>`, no text as image, link text naming its destination, and a text part built from the same document rather than by stripping tags.
  - **Met in full 2026-08-28 (B-198).** B-191 met every clause but the table: the schedule went out as a numbered list because there was no HTML body to put a table in. There still is not one, and there does not need to be — `plan.schedule` is now a structured merge value, so the same installments render as the numbered list in the text part and as a captioned table with `<th scope="col">` and `<th scope="row">` in the HTML part, through the same `tableHtml` a report email uses (D-102).
- AC: copy is **customer language** (D-15) — no "installment status", no "hold lifted", no "delinquency stage", no backlog ID. What the tenant needs is the amount, the date, and the next action.
- AC: the message and the portal page state the **same** figures from the same derivation — a break notice quoting a different balance from `/portal/payment-plan` is the call the office least wants to take.
...
- AC: Template editor per template (email: subject + body; SMS: text with segment counter and 160/segment warning), with merge-field picker restricted to that template's event schema; save is versioned; sends record template version.
...
- **Amended 2026-08-28, B-198, D-102 — one body, not two.** This AC read "subject + HTML/text body" and the schema carried a `bodyHtml` to match. It was null on every row the product ever had, and this editor wrote it null on every save, so the second body was never a thing an operator could edit — it was a thing a future seed would have lost. It is deleted. The HTML part of an email is the one body rendered as markup (paragraphs from blank lines, the subject as the single `<h1>`, every merged value escaped), and a field whose value is tabular carries its own table. An operator editing a template is therefore editing the whole message, both parts, which is also why the preview can keep showing one.
