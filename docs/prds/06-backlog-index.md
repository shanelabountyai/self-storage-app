...
**333 items — 316 complete, 17 open.**
...
| 83azw | B-324 | The two customer amount inputs are 36px tall | XS | B-112 ✅ |
| 83azx | B-326 | Three `ScrollRegion` names repeat their table's caption verbatim | XS | B-282 ✅ |
| 83azy | B-327 | A voided rent invoice's period can never be billed again | M | B-303 ✅ |
...
| 83ab | B-275 | ✅ | The Neon dev branch has a schema, no migration history, and 13 statements of drift | S | Phase 2 |
| 83ac | B-276 | ✅ | The auction lot can be sold at a venue the served notice never named | S | Phase 2 |
| 83aca | B-292 | ✅ | `reconcile()` called every lease that had paid an invoice a discrepancy — and the lien-notice gate… | S | MVP |
| 83ad | B-277 | ✅ | Nothing sweeps for leases whose ledger and invoices disagree — and two defects in this block leave… | M | Phase 2 |
| 83ae | B-278 | ✅ | The receipt names one arbitrary unit for a payment that settled several, and the nav sends a payer… | S | Phase 3 |
| 83af | B-279 | ✅ | A business account's payer is never sent a bill or a past-due notice | M | Phase 3 |
| 83ag | B-280 | ✅ | A business account cannot pay at the counter, and forcing it through mis-posts the money | S | Phase 3 |
| 83ah | B-281 | ✅ | The counter and the dunning ladder have no paper lane — and B-238 made that a supported renter | M | Phase 2 |
| 83ai | B-282 | ✅ | Three customer money-path tables bypass `ScrollRegion` | PRD 01 §6.8; PRD 02 §5.5; D-122; B-217, B-249, B-256 | B-249 ✅, B-256 ✅ |
| 83aia | B-293 | ✅ | `/admin/access` renders six cells under seven headers, so every gate result after "Who" sits under… | S | Phase 2 |
| 83aj | B-283 | ✅ | The Spanish payment reminder opens an English payment screen | S | Phase 2 |
| 83ak | B-284 | ✅ | The portal's Spanish has holes exactly on the money and irreversible screens | M | Phase 2 |
| 83al | B-285 | ✅ | `/portal/access`: the unlock button blurs itself, and two static paragraphs shout | XS | Phase 2 |
| 83am | B-286 | ✅ | The language toggle puts `lang` on the element whose accessible name is an `aria-label` in the… | XS | Phase 2 |
| 83an | B-287 | ✅ | Somebody granted sight of a business account is never told, and may not be able to sign in | S | Phase 3 |
| 83ao | B-288 | ✅ | Spanish calls the rented thing two different words | XS | Phase 2 |
| 83ap | B-289 | ✅ | The counter's shared-address warning is a policy memo, not a counter script | XS | Phase 2 |
| 83aq | B-290 | ✅ | Nothing ever offers Spanish; the visitor has to find it | S | Phase 2 |
| 83ar | B-291 | ✅ | Four public pages announce an English `<title>` under `<html lang="es">` | XS | Phase 2 |
| 83as | B-294 | ✅ | Five portal screens still announce an English `<title>` under `<html lang="es">` | XS | Phase 2 |
| 83at | B-295 | ✅ | Seven `role="alert"`s on the portal are page content, not status messages | XS | Phase 2 |
| 83au | B-296 | ✅ | The support-session log's default window ends before the session it was opened to check | XS | Phase 2 |
| 83av | B-297 | ✅ | A month report files the last evening of the month in the next month, and the accounting close… | S | Phase 2 |
| 83aw | B-298 | ✅ | D-138's audit finished: four more surfaces still reckoned a month in UTC, and `Lease.startDate`… | S | Phase 2 |
| 83ax | B-299 | ✅ | Four presses removed what the reader was standing on and moved focus nowhere | XS | Phase 2 |
| 83ay | B-300 | ✅ | The one auth email the recipient did not ask for told them to ignore it | XS | Phase 2 |
| 83aya | B-325 |  | `authAccountAccess` is one 62-word sentence doing three jobs | XS | Phase 3 |
| 83az | B-301 |  | Existing business-account members were never told they have portal access | S | Phase 2 |
| 83aza | B-302 | ✅ | The accessibility statement promises an error behaviour the two money screens do not have | S | Phase 2 |
| 83azb | B-303 | ✅ | A ledger exception cannot be repaired inside the product, so the lien gate that refuses it is… | L | Phase 2 |
| 83azc | B-304 | ✅ | One unrepairable lease raises a high-priority task every business day forever | S | Phase 2 |
| 83azd | B-305 | ✅ | At the counter, picking a unit for a business-account payer does not pay that unit | M | Phase 3 |
| 83aze | B-306 | ✅ | A refused lien notice leaves no record and no worklist | S | Phase 2 |
| 83azf | B-307 | ✅ | Every month that was already closed now disagrees with the code, and nothing tells anybody | M | Phase 2 |
| 83azg | B-308 | ✅ | The counter's shared-address question and its only button say opposite things | XS | Phase 2 |
...
| 83azs | B-320 |  | A card taken at the counter has no printable receipt, and a business account cannot pay by card at… | M | Phase 3 |
| 83azt | B-321 |  | The token landing pages reached from translated emails are English, and two of them dead-end | S | Phase 2 |
| 83azu | B-322 |  | A business-account payer is refused a prepayment with no reason and no way out | XS | Phase 3 |
| 83azv | B-323 |  | Two staff-facing strings print internal identifiers at the customer counter | XS | Phase 2 |
...
| 90 | B-090 | ✅ | Closed 2026-09-11: all six parts are built (B-090a–f), and the work they handed on is either ✅… | XL | Phase 3 |
