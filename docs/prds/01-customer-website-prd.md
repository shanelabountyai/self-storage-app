...
- **The three questions a first-timer asks are answered before they type anything** *(added 2026-09-24 from the first-time-renter review, findings 1, 3, 4 and 10, review block 12; B-397)*. "How bad is late", "how do I leave" and "what does protection cost" are answered today only at checkout step 4 (`lateFeeSummary`) or inside the portal. The "What you'd pay today" expander gains a **"If you pay late / If you leave"** block, every figure read from the configuration that already drives the lease and the engines, never typed: rent due day (or "on your move-in anniversary" under anniversary billing), the late-fee ladder's first step and its grace days (`lateFeeSummary`'s source), the day gate access pauses (US-45's day-N), "we write to you before anything is sold", and the facility's move-out notice days (`moveOutNoticeDays`, "no notice needed" when 0). The protection line gains the **price range of the facility's active tiers** ("$9–$19/mo, or show your own cover"). The checkout payment step states **when the gate code starts working** for a future-dated move-in, written from what provisioning actually does, which the build session determines and records rather than assumes. What protection covers and excludes, and a stated rate-change notice period, are **not** in this AC: both are legal copy and stay with §10 Q1/Q2 and the owner questions of review block 12. Testable: one assertion per figure that the page text equals the configured value for a seeded facility, and a second facility with different settings renders different text. **WCAG 2.1 AA:** the block is a `<dl>` inside the existing expander (1.3.1), every figure carries a unit or label (1.1.1 is n/a; the no-bare-number rule above), 4.5:1 text (1.4.3), and it reflows at 320px and under forced text spacing (1.4.10, 1.4.12), asserted through the facility page's existing state scan with the expander open.
...
- **A charged renter is never shown a card form again** *(added 2026-09-24 from the accessibility and first-time-renter reviews, review block 12; B-392)*. After `confirmPayment` succeeds the move-in finalises from the webhook (FR-4.4), and until it lands the renter must be told the money was taken. Once the client has a succeeded or processing PaymentIntent, or the server knows the charge is settling, the payment step renders a **"Payment received — confirming your unit"** section in place of the Payment Element, with its own heading, `tabIndex={-1}` and focus moved to it on arrival (**SC 2.4.3 Focus Order, A**). **The server's `settling` is read from Stripe as well as from the local `Payment` row**, because the row is itself written by the webhook — a lost webhook leaves the row `pending`, so a server that reads only the row mounts a live card form for a paid intent. If confirmation outlasts the bounded wait, the section says once, through a polite status region, that it is taking longer than usual and **the renter does not need to pay again**, with a "Check again" button and the facility phone (**SC 4.1.3 Status Messages, AA; SC 3.3.4 Error Prevention (Legal, Financial), AA**). Stripe's `payment_intent_unexpected_state` is never passed through as `result.error.message`; it maps to the same copy in both locales (**SC 3.3.1, A; SC 3.3.3, AA**). If Stripe.js fails to load or has not resolved after a bounded wait, the step renders the existing `pay.cardsUnavailable` sentence with the phone link, and pressing Pay before it resolves writes "The card form is still loading" into the status region rather than doing nothing silently (**SC 4.1.3**). Polling for the step must not create a PaymentIntent or write a `Payment` row per poll.
...
- **A bill that is not late does not look late, and one bill is shown once** *(added 2026-09-24 from the UX review, finding 3, review block 12; B-393)*. The dashboard's balance panel renders whenever `balanceCents > 0`, so an issued invoice not yet due shows "You have a balance of $95. Pay $95 now" above "Next payment $95 on Oct 1" and reads as $190 owed. The panel branches on the balance's state, from the same `daysPastDue` definition staff use (US-39's one-definition AC; B-375 fixed the staff half): **past due** — "$95 past due since Sep 1" with Pay as the primary action (the US-702 banner above still governs); **due, not late** — one line, "$95 due Oct 1", and the separate next-payment figure is not repeated when it is the same invoice; **autopay on and nothing past due** — "Autopay will charge $95 on Oct 1", with Pay demoted to a "Pay early" text link. **WCAG 2.1 AA:** state is carried by words, never colour alone (**SC 1.4.1, A**); the Pay-early link names its amount (**SC 2.4.4, A**); both locales.
...
- **The tenant says why they are leaving** *(added 2026-09-24 from the operator review, review block 12; B-399; PRD 02 US-14)*. Scheduling a move-out requires one reason from a short list (price or rate increase, moving away, no longer needed, bought a home, switched to another facility, a problem with the service, other with a text box of at most 200 characters). **WCAG 2.1 AA:** native radios in a `<fieldset>` with a `<legend>` (**SC 1.3.1, A**), the requirement stated in the legend rather than by an asterisk alone (**SC 3.3.2, A**), a refusal names the missing field and moves focus to it (**SC 3.3.1, A**), both locales.
...
- **Still open on 2026-09-24.** The UX review of review block 12 raised it again with a phone walk: name, photos, address, the only filled button (Call), two hours tables, the filter form and an open promo form all precede the first price. It is listed as an owner question in that block, not decided here. **B-396 changes what it can without answering it**: a "From $X/mo · N sizes available — See sizes" line in the header block anchored to `#units`, the promo form and the facility filter form each inside a `<details>` open only while a code or filter is applied (D-149's pattern), and the phone sticky bar reading "From $X/mo · See sizes" instead of renting the cheapest size. The hours tables do not move.
- **A search filter either filters or is not offered** *(added 2026-09-24 from the UX review, finding 2, review block 12; B-395)*. `SearchInput` carries only `size`, so the rail's feature checkboxes change nothing while the button says "Apply filters". Features are passed to the ranking: a facility with no available unit matching every ticked feature is dropped, and each card is priced from the cheapest available unit that matches. `search.carryingBefore/After` is deleted.
- **Search results carry what the facility card carries** *(same review, finding 5; B-401)*: today's hours, the open/closed badge and a "View units" button, by rendering `FacilityCard` or giving `ResultCard` the same parts. The badge reads **"Office open now" / "Office closed now"**, because the never-conflate rule above applies to a badge as much as to a table.
...
- Spanish-language support (highest-impact i18n).
  ...
  - *Built B-260, 2026-09-05 (**D-122**) — the portal.* All seventeen routes a signed-in tenant can reach, their nav, the shell and the components shared with checkout, so the renter B-090f sends to "Ir a mi cuenta" now arrives at a Spanish account. Three shared catalogues moved from words to message keys (`recurringParts` tokens, `SHARED_ACCESS_PRESETS`, `REFERRAL_STATE_LABELS`) so the tenant's portal and the English admin can render the same row in different languages, and the portal stopped inheriting the ADMIN transfer refusals — a correctness fix as much as a language one. `/portal` is scanned by axe in Spanish as a past-due tenant; the other ten portal routes are declared English-only in `STATE_EXCEPTIONS` and render on the accessibility statement.
  - *Built B-290, 2026-09-11 (**D-133**) — the offer.* A browser whose `Accept-Language` ranks Spanish above English, on a visit with no `st_locale` cookie, sees one Spanish line above the public header offering Spanish. It is never switched: accepting sets the cookie to `es`, declining sets it to `en`, and either answer ends the offer for good. URLs, `hreflang` and what a crawler sees are unchanged (D-122). The portal, `/login` and pay links carry no offer; the first two are reached from the public site, and a pay link already speaks its reminder's language (B-283).
  - *Built B-291, 2026-09-11 (**D-134**) — page titles.* `<title>` follows the reader on `/faq`, `/about`, `/contact`, `/accessibility` and `/messaging-policy`, because it is the page name a screen reader announces first (SC 3.1.2). Each page takes it from its `<h1>` key, whose English is the title it always had, so what a crawler reads as the title did not change. `description`, `alternates` and Open Graph stay English (D-122, D-123). Five portal screens still set an English `<title>`, with no owning row.
  - *Built B-294, 2026-09-11 (**D-134**) — portal page titles.* Those five (`/portal/contact`, `/portal/documents`, `/portal/documents/[documentId]`, `/portal/protection`, `/portal/statements/account/[accountId]/[period]`) now take `<title>` from a message key, so every portal screen's `<title>` follows the reader. One document keeps the generic title rather than its stored one, and the account statement has its own `astmt.title` because its `<h1>` names the account.
  - *Open after B-090f and B-260:* the three consent disclosures on checkout step 1 are still English and deliberately so — a translated TCPA/E-SIGN disclosure recorded against an English version constant is evidence of a consent nobody gave (**B-259**). The Spanish stops at the signed-in product: the static and legal pages, the guides, and the city and size landing pages with their generated SEO prose are all still English, and the guides need an `hreflang` answer before translating them means anything (**B-262**). And the cookie is a browser preference, not a durable one — every email and text, including the dunning ladder, still goes out in English to a renter who rented in Spanish (**B-261**).
...
- Business accounts (multi-unit consolidated billing, additional authorized users).
  ...
  - *Built B-279, 2026-09-10 (**D-136**) — the payer's mail.* The payer receives the bill (`invoice.due_soon`, `invoice.due_today`) and the courtesy past-due ladder (`delinquency.day_reached`) as a SECOND recipient beside the lease's tenant, in the payer's own language and with a pay link of their own. The lien and pre-lien supplements, the overlock and gate notices, and anything about the tenant's own card stay with the tenant alone (**D-118**, **D-119**). `/admin/billing/accounts/[id]` now shows days past due and the furthest ladder stage in its summary.
  - *Built B-287, 2026-09-11 — a member is told.* Adding a member emails them the password-reset link with one sentence in front: which account they can now see, and that the link is needed only if they have no password. It is written in the member's `preferredLocale`, else English, and never in the staffer's browser language. The send runs after the commit and cannot undo it. If it fails, the staffer's confirmation says the email did not go and tells them to point the member at "Forgot your password?". This is notification only: `payableLeaseWhere` is unchanged (D-120).
  - *Settled 2026-09-19 by the review block over B-301–B-328 (**B-330**, built 2026-09-21) — what a payment made against the ACCOUNT may settle:* **only the account's own leases.** `payableLeaseFilter`'s union (D-118) is what lets a payer who also rents a unit personally pay for it, and it stays exactly that for a payment made in their own name. A payment made against the account — at the counter, by card, or on `/portal/pay?account=` — is restricted to the account's open invoices, with any surplus as credit on the anchor lease. Until then an account's check could settle the payer's personal unit because it was older, while the screen quoted and promised the account's units.
  - *Settled 2026-09-21 by the review block over B-329–B-349 (**B-350**) — how much a payment made against the ACCOUNT may be:* **no more than the account's open balance, at the counter as on the portal (B-256).** Every job that spends unapplied credit looks it up by the lease's tenant, and the account's units are held in the employees' names, so a surplus recorded in the payer's name is spent on the payer's own unit or never spent at all. Refusing the surplus decides nothing about whose credit an account credit would be; that stays **D-113**'s.
  - *Stated limit, recorded 2026-09-14 from the operator review of B-275–B-300 — **a known limit, not a defect, and deliberately not a backlog row**:* **a business account cannot span facilities.** `BillingAccount` carries `facilityId` with `@@unique([facilityId, name])` and the composite foreign key that pins each lease to the account's own facility (`packages/db/prisma/schema.prisma:1545-1571`), and `counterPayableAccounts` and `accountAnchor` are both facility-scoped (`apps/web/lib/admin/pos.ts:894-960, 362-391`). **So a contractor with units at three of your sites is three accounts, three statements, three counter payments and three sets of reminders.** That composite key is load-bearing and was chosen on purpose — `Payment.facilityId` is not nullable, and a cross-site attachment would settle out of the wrong drawer and the wrong accounting period (D-118) — so this is defensible for a single-site product and wrong for the 40-site portfolio the platform is aimed at. Removing it is a schema change with a large blast radius across payments, the drawer, the close and every account screen, and the reviewer explicitly declined to raise it as a row today. It is written here so the next reviewer meets it as a stated limit rather than re-finding it as a defect; **a portfolio-level account is new scope and needs an owner decision and a D-number before anyone builds toward it.**
