...
- **The insurance half was built in B-104** at `/portal/protection`. A tier change is **scheduled to the start of the next billing period, never applied today** — even when today is the billing day, because that period's invoice may already have been raised. The reason is not tidiness: a protection premium is a flat monthly charge, so changing it mid-period would mean prorating one, and a prorated premium is a coverage question rather than an arithmetic one ("was this unit covered to $2,000 or $5,000 on the 14th?"). It also stops a tenant upgrading the morning after a break-in and having it apply to the month just gone. **Dropping a paid plan requires current, unexpired proof of the tenant's own cover** — an expired policy is not cover, and letting one justify the drop is the gap D-17 exists to close. **No file is uploaded**: there is no blob store in this project, so what is captured is the insurer, the policy number and the expiry date — the last of which is what D-17's nightly lapse scan actually reads — plus a staff task to check them against the declaration page.
