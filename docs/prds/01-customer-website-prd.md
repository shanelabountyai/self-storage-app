...
- Business accounts (multi-unit consolidated billing, additional authorized users).
  ...
  - *Open after B-090e:* the portal still shows one card and one Pay button **per lease**, additional authorized users are unbuilt (and collide with **D-111**'s unique `Tenant.email`), and **autopay still charges each lease's own tenant, not the payer** — whether attaching a lease moves that mandate is an owner decision, not a build one. All three are **B-256**.
