...
- Business accounts (multi-unit consolidated billing, additional authorized users).
  ...
  - *Open after B-256:* **additional authorized users are unbuilt** — an account still has exactly one person who can see it, so giving the office manager sight of it means making them the payer. It needs a `BillingAccountMember` table, its control, and `payableLeaseWhere`/`resolveAudience` learning that membership reaches a lease; it collides with **D-111**'s unique `Tenant.email`, which refuses two members at one household address. That is **B-258**.
