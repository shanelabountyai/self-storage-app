...
- **Accessibility:** the dashboard card is a live region present at page load rather than inserted on change (4.1.3 AA); no plan state is carried by colour alone (1.4.1 A); the schedule is a real `<table>` with a `<caption>` and column headers (1.3.1 A); copy is customer language throughout (D-15).
