...
export type PromotionCandidate = PromotionTerms & {
  ...
  /// FR-PROMO-1's min-stay condition, in months. `0` means none. Appended to
  /// the terms whichever way the wording was arrived at (B-144) — it is a
  /// condition of the offer, not part of its description.
...
export function evaluatePromotions(
  ...
  if (
    typed &&
    applicable.every((result) => result.code === null) &&
    !rejection
  ) {
  ...
  return {
    best,
    automatic: applicable.filter(
      (result) => result.promotion.displayMode === "auto",
    ),
...
function outcomeFor(
  ...
  if (best && best.code === null)
    return { kind: "superseded", keptTerms: best.terms };
...
function disqualify(
  ...
  if (promotion.startsAt && context.at < promotion.startsAt)
    return "window_closed";
  if (promotion.endsAt && context.at >= promotion.endsAt)
    return "window_closed";

  if (
    promotion.facilityIds.length > 0 &&
    !promotion.facilityIds.includes(context.facilityId)
  ) {
    return "not_for_this_facility";
  }
  if (
    promotion.unitTypeIds.length > 0 &&
    !promotion.unitTypeIds.includes(context.unitTypeId)
  ) {
    return "not_for_this_size";
  }
  if (promotion.newTenantOnly && !context.isNewTenant)
