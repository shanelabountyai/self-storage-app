...
**B-306 is done** (`279bf74`, SHA recorded in the follow-up commit). The eighth review block (`5933447`) has 21 rows left, **B-307–B-326** plus **B-327**, and **B-325 at `83aya`** (deliberately ahead of B-301 so mail's wording settles before the backfill sends).

A refused `generateNotice` now writes a `notice.refused` audit entry and raises one open card per lease per notice type; generating the notice cancels the card, and no note can. **No migration** — the evidence is an `audit_log` row and the worklist is a catalog `Task` type. **`resolvedByAction.href` and `linkLabel` are now optional** in the task catalog; a type whose destination is scoped to one row renders the sentence alone.
