...
**B-308 is done** (`e26f897`, SHA recorded in the follow-up commit). The eighth review block (`5933447`) has 19 rows left, **B-309–B-326** plus **B-327**, and **B-325 at `83aya`** (deliberately ahead of B-301 so mail's wording settles before the backfill sends).

The counter's shared-address button said `Yes, give {renting} their own account` and was the **no** branch. It now says `No — different person, rent to {renting}` in both languages, and `tests/i18n.test.ts` guards the class rather than the string: when `details.sharedEmail` poses a question, the confirm label must not open with that language's affirmative, nor contain account/cuenta. **Copy only — no code, no migration.** The branch itself was always correct.

## Start here

**B-309**, in file order.

- **B-309** — the business-account payer gets N bills, N pay links, and none of them opens the account (digital-experience review 2026-09-14, finding 1 merged with operator review finding 9; verified in code).
