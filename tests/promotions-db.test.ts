...
import {
  discountForLeasePeriod,
  markDiscountApplied,
} from "../apps/web/lib/promotions/billing";
import { leaseValuesFor } from "../apps/web/lib/lease/build";
import { createPromotion } from "../apps/web/lib/admin/promotions";
...
describeDb("promotions", () => {
  beforeAll(async () => {
    ...
    const tenant = await prisma.tenant.create({
      data: {
        email: `promo-${suffix}@example.com`,
        firstName: "Ada",
        lastName: "Renter",
      },
    });
  ...
  beforeEach(async () => {
    ...
    await prisma.promoCode.deleteMany({
      where: { promotion: { name: { contains: suffix } } },
    });
  ...
  describe("redeemPromotion — FR-PROMO-5", () => {
    it("claims a redemption and snapshots the schedule", async () => {
      ...
      const after = await prisma.promotion.findUniqueOrThrow({
  ...
  describe("the billing hand-off — US-12 AC2", () => {
    it("answers what comes off each period from the snapshot", async () => {
      ...
      expect((await discountForLeasePeriod(leaseId, 0))?.amountCents).toBe(
        6_450,
      );
      expect((await discountForLeasePeriod(leaseId, 1))?.amountCents).toBe(
        6_450,
      );
    ...
    it("never discounts the same period twice, however often the run repeats", async () => {
      ...
      await prisma.$transaction((tx) =>
        markDiscountApplied(tx, redeemed.redemptionId, 0),
      );
  ...
  describe("creating one — US-10 (B-144)", () => {
    function settingsActor(): Actor {
      return {
        kind: "staff",
        staffUserId: "staff-promo-test",
        assignments: [
          {
            facilityId,
            roleKey: "manager",
    ...
    it("refuses a minimum shorter than the discount it is meant to protect", async () => {
      // "First three months free, one-month minimum" gives away three months
      // and holds the tenant to one — a condition that protects nothing, and
      // one B-145 could never recover from. Refused at the form, not discovered
      // on a final statement.
      const result = await createPromotion(settingsActor(), facilityId, {
        name: `Bad minimum ${suffix}`,
        type: "free_months",
        value: 0,
        durationPeriods: 3,
        displayMode: "auto",
      ...
      expect(result).toEqual({
        ok: false,
        field: "minStayMonths",
        problem: expect.stringContaining("3 months"),
      });
      // Refused before anything was written, not written and then rolled back.
      expect(
        await prisma.promotion.count({
          where: { name: `Bad minimum ${suffix}` },
        }),
      ).toBe(0);
    });
  });
  ...
  describe("the advertised price and the charged price (found 2026-08-14)", () => {
    ...
    async function availableUnit() {
      ...
      await prisma.unit.create({
        data: {
          facilityId,
          unitTypeId,
          number: `PC-${suffix.slice(0, 3)}-${sequence}`,
        },
      });
    }
    ...
    it("carries a minimum stay onto the checkout summary and the lease (B-144)", async () => {
      // `minStayMonths` shipped with B-070 and NOTHING could set it — a
      // repo-wide grep found it only in `schema.prisma`. This walks the whole
      // path the operator's new form field feeds: offer → session snapshot →
      // the promo line the price summary labels → the signed agreement.
      await availableUnit();
      await makePromotion({
        type: "free_months",
        value: 0,
      ...
      // The label the price summary renders against the discount line, read
      // from the snapshot rather than re-evaluated — so what the renter agreed
      // to cannot drift from what they were shown.
      const due = await amountDueToday(session);
      expect(due.lines.find((line) => line.key === "promo")?.label).toBe(
        "First month free — 6-month minimum stay",
      );

      // And the signed document. Section 3 used to state flatly that there was
      // "no penalty for leaving", which a minimum stay contradicts — in the
      // tenant's favour, on the one artefact that decides who is right.
    ...
    it("leaves the lease saying there is no penalty when no minimum applies (B-144)", async () => {
      ...
      if (!started.ok) throw new Error("no unit to start a checkout on");
      const values = await leaseValuesFor(
        (await sessionByToken(started.token))!,
      );
      expect(values.termSummary).toBe(
    ...
    it("locks the offer, so pausing the promotion mid-checkout cannot raise the total", async () => {
      ...
      expect(due.totalDueTodayCents).toBe(RENT - RENT / 2);
      expect(promoDiscountOn(session!)?.schedule).toEqual(
        offer.offer!.schedule,
      );
    });
    ...
    it("increments the code’s own usesCount when the move-in redeems it", async () => {
      ...
      const after = await prisma.promoCode.findUniqueOrThrow({
        where: { id: code.id },
      });
    ...
    it("keeps the better automatic offer when the typed code is worth less", async () => {
      ...
      const gated = await makePromotion({
        name: `Gated ${suffix}`,
        displayMode: "code",
        value: 10,
      });
