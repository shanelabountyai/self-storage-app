...
describeDb('inbound SMS routing (B-135)', () => {
  beforeAll(async () => {
    ...
    const unit = await prisma.unit.create({
      data: { facilityId, unitTypeId: unitType.id, number: `S-${suffix.slice(0, 4)}`, status: 'available' },
    })
  ...
  afterEach(async () => {
    ...
    await prisma.domainEvent.deleteMany({ where: { name: 'sms.inbound_received' } })
  })
  ...
  it('records the words and raises a task at the tenant’s facility', async () => {
    const result = await routeInboundSms({ rawPhone: TENANT_PHONE, body: 'the gate won’t open' })
    ...
    const payload = event.payload as { body: string; tenantId: string | null; phone: string }
  ...
  it('does not swallow a second message the same day', async () => {
    ...
    const first = await routeInboundSms({ rawPhone: TENANT_PHONE, body: 'I paid yesterday' })
    const second = await routeInboundSms({ rawPhone: TENANT_PHONE, body: 'also my code stopped working' })
    expect(first.ok && second.ok).toBe(true)
    if (!first.ok || !second.ok) throw new Error('unreachable')
    expect(first.taskId).not.toBe(second.taskId)

    expect(await prisma.task.count({ where: { facilityId, type: 'inbound_sms_review' } })).toBe(2)
    ...
    expect(bodies).toEqual(['I paid yesterday', 'also my code stopped working'])
  })

  it('still records a message from a number it cannot place, without a task', async () => {
    const result = await routeInboundSms({ rawPhone: STRANGER_PHONE, body: 'do you have a 10x20?' })
    ...
    expect((event.payload as { body: string }).body).toBe('do you have a 10x20?')
    expect(event.facilityId).toBeNull()
    expect(await prisma.task.count({ where: { type: 'inbound_sms_review', facilityId } })).toBe(0)
  })

  it('falls back to the SMS line when the facility has no phone on file', async () => {
    await prisma.facility.update({ where: { id: facilityId }, data: { phone: null } })
    try {
      const result = await routeInboundSms({ rawPhone: TENANT_PHONE, body: 'hello?' })
      if (!result.ok) throw new Error('unreachable')
      expect(result.facilityPhone).toBeNull()
    } finally {
      await prisma.facility.update({ where: { id: facilityId }, data: { phone: '512-555-0100' } })
    }
  })

  it('ignores an empty body rather than claiming a person wrote something', async () => {
    expect(await routeInboundSms({ rawPhone: TENANT_PHONE, body: '   ' })).toEqual({
      ok: false,
      reason: 'empty',
    })
    ...
    expect(
      await prisma.domainEvent.count({ where: { name: 'sms.inbound_received', facilityId } }),
    ).toBe(0)
  })

  it('puts the message on the task card, so nobody opens it to triage', async () => {
    const result = await routeInboundSms({ rawPhone: TENANT_PHONE, body: 'my unit has water in it' })
    if (!result.ok) throw new Error('unreachable')
    const task = await prisma.task.findUniqueOrThrow({ where: { id: result.taskId } })
  ...
  it('truncates a rambling message rather than letting it push the queue off screen', async () => {
    ...
    const result = await routeInboundSms({ rawPhone: TENANT_PHONE, body: long })
    if (!result.ok) throw new Error('unreachable')
    const task = await prisma.task.findUniqueOrThrow({ where: { id: result.taskId } })

    const subject = (await resolveTaskSubjects([task])).get(`DomainEvent:${task.entityId}`)
    ...
    const event = await prisma.domainEvent.findUniqueOrThrow({ where: { id: task.entityId } })
