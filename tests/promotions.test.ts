...
describe("discountSchedule — FR-PROMO-1", () => {
  it("makes first-month-free the whole rent, once", () => {
    const schedule = discountSchedule(
      { type: "free_months", value: 0, durationPeriods: 1 },
      RENT,
    );
  ...
  it("spreads a percentage across the periods it covers", () => {
    const schedule = discountSchedule(
      { type: "percent_off", value: 50, durationPeriods: 3 },
      RENT,
    );
    expect(schedule.periods.map((period) => period.periodIndex)).toEqual([
      0, 1, 2,
    ]);
  ...
  it("rounds the discount, so rent minus discount is whole cents", () => {
    ...
    const schedule = discountSchedule(
      { type: "percent_off", value: 33, durationPeriods: 1 },
      9_999,
    );
  ...
  it("clamps a percentage above 100 and below 0", () => {
    expect(
      discountSchedule(
        { type: "percent_off", value: 150, durationPeriods: 1 },
        RENT,
      ).totalCents,
    ).toBe(RENT);
    expect(
      discountSchedule(
        { type: "percent_off", value: -10, durationPeriods: 1 },
        RENT,
      ).totalCents,
    ).toBe(0);
  });
  ...
  it("produces nothing for a zero-length promo", () => {
    expect(
      discountSchedule(
        { type: "free_months", value: 0, durationPeriods: 0 },
        RENT,
      ).periods,
    ).toEqual([]);
  });
  ...
  it("answers what comes off a given period", () => {
    const schedule = discountSchedule(
      { type: "percent_off", value: 50, durationPeriods: 2 },
      RENT,
    );
...
describe("minimum stay — FR-PROMO-1 (B-144)", () => {
  // The column existed from B-070 and NOTHING could set it — verified by a
  // repo-wide grep that found `minStayMonths` only in `schema.prisma`. So
  // "first month free with a six-month minimum" could not be expressed at all:
  ...
  it("survives an operator writing their own terms", () => {
    // The regression this exists for. `termsText` WINS over the generated
    // sentence, so a minimum folded into `describeTerms` would vanish the
    // moment somebody typed their own wording — quietly dropping a condition
    // the renter is held to and B-145 charges money on.
    const [result] = evaluatePromotions(
      [
        promo({
          type: "free_months",
          durationPeriods: 1,
          termsText: "First month FREE!",
          minStayMonths: 6,
        }),
      ],
      context,
...
describe("evaluatePromotions — FR-PROMO-3", () => {
  ...
  it("hides a code-gated promo until its code is typed", () => {
    ...
    const withCode = evaluatePromotions([gated], {
      ...context,
      code: "SPRING25",
    });
  ...
  it("matches codes case-insensitively — FR-PROMO-2", () => {
    ...
    for (const typed of ["spring25", "SPRING25", " Spring25 "]) {
      expect(
        evaluatePromotions([gated], { ...context, code: typed }).best,
      ).not.toBeNull();
    }
  });
  ...
  it("tells a prospect why their code did not work", () => {
    ...
    const wrongFacility = promo({
      displayMode: "code",
      codes: ["other"],
      facilityIds: ["f2"],
    });
    const result = evaluatePromotions([wrongFacility], {
      ...context,
      code: "other",
    });
  ...
  it("reports an unknown code as unknown", () => {
    expect(
      evaluatePromotions([promo()], { ...context, code: "nope" }).codeOutcome,
    ).toEqual({
      kind: "rejected",
  ...
  it("keeps the better automatic offer and says so, rather than silently ignoring the code", () => {
    ...
    const result = evaluatePromotions([generous, stingy], {
      ...context,
      code: "meh",
    });
  ...
  it("reports a code that beats the automatic offer as applied", () => {
    ...
    const result = evaluatePromotions([stingy, generous], {
      ...context,
      code: "big",
    });
  ...
  it("respects targeting, windows and the new-tenant flag", () => {
    expect(
      evaluatePromotions([promo({ facilityIds: ["f2"] })], context).best,
    ).toBeNull();
    expect(
      evaluatePromotions([promo({ unitTypeIds: ["u2"] })], context).best,
    ).toBeNull();
    expect(
      evaluatePromotions([promo({ newTenantOnly: true })], {
        ...context,
        isNewTenant: false,
      }).best,
    ).toBeNull();
    expect(
      evaluatePromotions(
        [promo({ endsAt: new Date("2026-06-01T00:00:00Z") })],
        context,
      ).best,
    ).toBeNull();
    expect(
      evaluatePromotions(
        [promo({ startsAt: new Date("2026-07-01T00:00:00Z") })],
        context,
      ).best,
    ).toBeNull();
  });
  ...
  it("ignores a draft or paused promo", () => {
    expect(
      evaluatePromotions([promo({ status: "draft" })], context).best,
    ).toBeNull();
    expect(
      evaluatePromotions([promo({ status: "paused" })], context).best,
    ).toBeNull();
  });
  ...
  it("stops showing a promo that is already fully redeemed", () => {
    expect(
      evaluatePromotions(
        [promo({ maxRedemptions: 10, redemptionCount: 10 })],
        context,
      ).best,
    ).toBeNull();
  });
  ...
  it("picks the most valuable when several apply", () => {
    ...
    expect(evaluatePromotions([small, big], context).best?.promotion.id).toBe(
      "big",
    );
  });
  ...
  it("keeps the badge a prospect was already reading when a code ties it", () => {
    ...
    const result = evaluatePromotions([shown, typed], {
      ...context,
      code: "same",
    });
