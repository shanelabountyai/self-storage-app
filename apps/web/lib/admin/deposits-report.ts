...
export async function depositsReport(
  ...
  const facilities = facilityId
    ? allowed.filter((f) => f.id === facilityId)
    : allowed;
  if (facilities.length === 0)
    return { rows: [], totalVarianceCents: 0, totalUnreconciledCents: 0 };
  ...
  const [payments, sessions] = await Promise.all([
    prisma.payment.findMany({
      where: {
        ...
        // B-146: `returned` included — a deposit is a historical fact about
        // what was banked, not a claim that it cleared.
        status: {
  ...
  type Bucket = Omit<
    DepositsRow,
    "facilityId" | "facilityName" | "businessDate"
  >;
  ...
  for (const payment of payments) {
    ...
    const signed = payment.refundOfPaymentId
      ? -payment.amountCents
      : payment.amountCents;

    if (payment.method === "cash") {
      bucket.cashRecordedCents += signed - (payment.changeCents ?? 0);
      if (!payment.drawerSessionId)
        bucket.unreconciledCents += signed - (payment.changeCents ?? 0);
  ...
  for (const session of sessions) {
    ...
    bucket.countedCashCents =
      (bucket.countedCashCents ?? 0) + (session.countedCashCents ?? 0);
    bucket.expectedCashCents =
      (bucket.expectedCashCents ?? 0) + (session.expectedCashCents ?? 0);
    bucket.varianceCents =
      (bucket.varianceCents ?? 0) + (session.varianceCents ?? 0);
    if (session.note)
      bucket.note = bucket.note
        ? `${bucket.note}; ${session.note}`
        : session.note;
  ...
  return {
    rows,
    totalVarianceCents: rows.reduce(
      (sum, row) => sum + (row.varianceCents ?? 0),
      0,
    ),
    totalUnreconciledCents: rows.reduce(
      (sum, row) => sum + row.unreconciledCents,
      0,
    ),
  };
}
