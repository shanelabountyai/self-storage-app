...
import { describeTerms, discountSchedule } from '@storage/core/promotions'
...
export type PromotionRow = {
  ...
  terms: string
  codes: { id: string; code: string; expiresAt: Date | null; maxUses: number | null; usesCount: number }[]
  /// FR-PROMO-4's ROI half: what this promo has actually given away.
  discountedCents: number
}

export async function promotionsFor(actor: Actor, facilityId: string): Promise<PromotionRow[]> {
  ...
  return promotions.map((promotion) => ({
    ...
    terms: promotion.termsText?.trim() || describeTerms(promotion),
    ...
    discountedCents: promotion.redemptions.reduce((sum, row) => sum + row.totalCents, 0),
  }))
}

export type PromotionWriteResult = { ok: true; id: string } | { ok: false; field: string; problem: string }
...
export async function createPromotion(
  ...
  if (!input.name.trim()) return { ok: false, field: 'name', problem: 'Give it a name staff will recognise.' }
  if (input.type === 'percent_off' && (input.value < 1 || input.value > 100)) {
    return { ok: false, field: 'value', problem: 'A percentage between 1 and 100.' }
  }
  if (input.type === 'amount_off' && input.value < 1) {
    return { ok: false, field: 'value', problem: 'An amount greater than zero.' }
  }
  if (input.durationPeriods < 1) {
    return { ok: false, field: 'durationPeriods', problem: 'At least one month.' }
  }
  if (input.startsAt && input.endsAt && input.endsAt <= input.startsAt) {
    return { ok: false, field: 'endsAt', problem: 'The end has to come after the start.' }
  }
  ...
  await recordAudit({
    ...
    facilityId,
    context: { created: input.name.trim(), type: input.type, value: input.value },
  })
...
export async function setPromotionStatus(
  ...
  const promotion = await prisma.promotion.findUniqueOrThrow({
    ...
    select: { name: true, status: true, displayMode: true, facilityIds: true, unitTypeIds: true, codes: { select: { id: true } } },
  })
  ...
  if (status === 'active' && promotion.displayMode === 'code' && promotion.codes.length === 0) {
    return {
      ok: false,
      field: 'status',
      problem: 'This is a code-gated promotion with no codes. Add one before activating, or nobody can use it.',
    }
  }
...
export async function addPromoCode(
  actor: Actor,
  facilityId: string,
  input: { promotionId: string; code: string; maxUses: number | null; expiresAt: Date | null },
): Promise<PromotionWriteResult> {
  requirePermission(actor, 'facility:settings', facilityId)
  ...
  if (!/^[a-z0-9-]{3,32}$/.test(code)) {
    return {
      ok: false,
      field: 'code',
      problem: 'Letters, numbers and hyphens, 3 to 32 characters — it has to survive being read down a phone.',
    }
  }

  const existing = await prisma.promoCode.findUnique({ where: { code }, select: { id: true } })
  if (existing) return { ok: false, field: 'code', problem: 'That code is already in use.' }
