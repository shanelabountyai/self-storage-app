...
export async function resolveTaskSubjects(
  ...
  if (leaseIds) {
    ...
    for (const lease of leases) {
      result.set(key('Lease', lease.id), leaseSubject(lease.tenantId, lease.tenant, lease.unit.number))
    }
  }
  ...
  if (invoiceIds) {
    ...
    for (const invoice of invoices) {
      result.set(
        ...
        leaseSubject(invoice.lease.tenantId, invoice.lease.tenant, invoice.lease.unit.number),
      )
    }
  }
  ...
  if (gateCommandIds) {
    for (const [id, subject] of await resolveGateCommandSubjects([...gateCommandIds])) {
...
function leaseSubject(
  ...
  return { label: `Unit ${unitNumber} — ${tenant.firstName} ${tenant.lastName}`, href: `/admin/tenants/${tenantId}` }
}
...
async function resolveGateCommandSubjects(commandIds: string[]): Promise<Map<string, TaskSubject>> {
  const commands = await prisma.gateCommand.findMany({
    where: { id: { in: commandIds } },
    select: { id: true, grantId: true, credentialId: true },
  })

  const credentialIds = commands.map((c) => c.credentialId).filter((id): id is string => id !== null)
  const credentials = credentialIds.length
    ? await prisma.accessCredential.findMany({
        where: { id: { in: credentialIds } },
        select: {
          id: true,
          grant: { select: { tenantId: true, tenant: { select: { firstName: true, lastName: true } } } },
        },
      })
    : []
  ...
  for (const command of commands) {
    const viaCredential = command.credentialId ? credentialById.get(command.credentialId) : undefined
    const grant = viaCredential ? viaCredential.grant : command.grantId ? grantById.get(command.grantId) : undefined

    result.set(
      command.id,
      grant?.tenant
        ? { label: `${grant.tenant.firstName} ${grant.tenant.lastName}`, href: `/admin/tenants/${grant.tenantId}` }
...
export function fallbackSubject(entityType: string): TaskSubject {
  return { label: MISSING_SUBJECT_LABEL[entityType] ?? 'The subject of this task no longer exists.', href: null }
}
