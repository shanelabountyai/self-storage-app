...
/// override.
export async function tenantMoveOutLeases(tenantId: string): Promise<PortalMoveOutLease[]> {
  const leases = await prisma.lease.findMany({
    ...
    select: {
      ...
      moveOutDate: true,
      facility: { select: { name: true, phone: true, moveOutNoticeDays: true } },
...
export type PreviewResult = { ok: true; preview: PortalMoveOutPreview } | { ok: false; reason: 'not_found' }
...
export async function requestMoveOut(
  ...
  const lease = await prisma.lease.findFirst({
    where: { id: leaseId, tenantId, status: { in: [...OCCUPYING_LEASE_STATUSES] } },
    select: { facilityId: true, moveOutDate: true, facility: { select: { moveOutNoticeDays: true } } },
  })
  if (!lease) return { ok: false, reason: 'not_found' }
  ...
  if (moveOutDate.getTime() < minDate.getTime()) return { ok: false, reason: 'date_too_soon' }

  await prisma.$transaction(async (tx) => {
    await tx.lease.update({
      where: { id: leaseId },
      data: { moveOutDate, moveOutReason: 'tenant_request', noticeGivenAt: new Date() },
    })
    await createTask(
      {
        facilityId: lease.facilityId,
...
export type CancelMoveOutResult = { ok: true } | { ok: false; reason: 'not_found' | 'nothing_to_cancel' | 'too_late' }
...
export async function cancelMoveOutRequest(tenantId: string, leaseId: string): Promise<CancelMoveOutResult> {
  const lease = await prisma.lease.findFirst({
    where: { id: leaseId, tenantId, status: { in: [...OCCUPYING_LEASE_STATUSES] } },
