...
export default async function TemplatesPage({
  ...
  return (
    <div className="flex max-w-5xl flex-col gap-6">
      ...
      {/* B-329 / SC 3.1.2. `lang={option}` on each link, because the label IS
      ...
      {!active ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
          <AdminForm action={saveTemplateAction} label={`Edit ${active.key}`} className="flex flex-col gap-3">
            ...
            {/* B-329 / SC 3.1.2. The `lang` goes on the CONTROL, not on the
            ...
            <section aria-labelledby="preview-heading">
              ...
              {preview?.ok ? (
                <div className="border-input mt-2 rounded-lg border p-3 text-xs">
                  ...
                  {/* B-329. The row named the subject input, the body textarea
