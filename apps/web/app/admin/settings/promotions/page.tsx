...
import { addCodeAction, createPromotionAction, setStatusAction } from './actions'
...
export default async function PromotionsPage() {
  ...
  if (selected.mode !== 'single') {
    return (
      <p className="text-muted-foreground text-sm">
        Pick a specific facility above — a promotion is created against one site.
      </p>
    )
  }
  if (!hasPermissionAnywhere(actor, ['facility:settings'])) {
    return <p className="text-muted-foreground text-sm">You don&apos;t have access to promotions.</p>
  }
  ...
  return (
    <div className="flex max-w-4xl flex-col gap-8">
      <div>
        <h1 className="text-lg font-semibold">Promotions — {selected.facility.name}</h1>
        <p className="text-muted-foreground mt-1 max-w-prose text-sm text-pretty">
          A discount shown on unit cards and carried through checkout. New promotions are created as
          drafts — nothing reaches a customer until you activate it.
        </p>
      </div>

      <ul className="flex flex-col gap-4">
        {promotions.map((promotion) => (
          <li key={promotion.id} className="border-input flex flex-col gap-3 rounded-lg border p-4">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <span>
                <span className="font-medium">{promotion.name}</span>
                {/* Never colour alone (WCAG 1.4.1) — the status is words. */}
                <span className="text-muted-foreground"> · {STATUS_LABELS[promotion.status]}</span>
              </span>
              <span className="text-muted-foreground text-sm">
                {promotion.redemptionCount}
                {promotion.maxRedemptions !== null ? ` of ${promotion.maxRedemptions}` : ''} redeemed
                {promotion.discountedCents > 0 && ` · ${formatCents(promotion.discountedCents)} given away`}
              </span>
            </div>

            <p className="text-sm">{promotion.terms}</p>
            <p className="text-muted-foreground text-xs">
              {promotion.displayMode === 'code' ? 'Needs a code' : 'Shown automatically'}
            ...
            {promotion.codes.length > 0 && (
              <ul className="flex flex-wrap gap-2">
                {promotion.codes.map((code) => (
                  <li key={code.id} className="border-input rounded-md border px-2 py-1 text-xs">
                    <span className="font-mono uppercase">{code.code}</span>
                    <span className="text-muted-foreground">
                      {' '}
                      · {code.usesCount}
                      {code.maxUses !== null ? `/${code.maxUses}` : ''} used
                      {code.expiresAt ? ` · to ${formatDate(code.expiresAt)}` : ''}
                    </span>
                  </li>
                ))}
              </ul>
                ...
                .map((status) => (
                  ...
                  >
                    ...
                    <Button type="submit" variant="outline">
                      {status === 'active' ? 'Make live' : status === 'paused' ? 'Pause' : 'End'}
                    </Button>
                  </AdminForm>
                ))}
            </div>
            ...
            {promotion.displayMode === 'code' && (
              ...
              >
                <input type="hidden" name="facilityId" value={facilityId} />
                <input type="hidden" name="promotionId" value={promotion.id} />
                <Field name="code" label="Code" hint="Leave empty for a random one." className="min-w-40" />
                <Field name="maxUses" label="Max uses" type="number" min={1} className="w-28" />
                <Field name="expiresAt" label="Expires" type="date" className="w-44" />
                <Button type="submit" variant="outline">
                  Add code
                </Button>
              </AdminForm>
      ...
      >
        ...
        <Field name="name" label="Name" hint="What staff will call it. Customers see the terms below." />

        <div className="grid gap-3 sm:grid-cols-3">
          <Field name="type" label="Type" as="select" defaultValue="percent_off">
          ...
          />
          <Field name="durationPeriods" label="Months it covers" type="number" min={1} defaultValue="1" />
        </div>

        <div className="grid gap-3 sm:grid-cols-3">
          <Field name="displayMode" label="How it applies" as="select" defaultValue="auto">
