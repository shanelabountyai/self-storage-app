...
export async function createPromotionAction(
  ...
  const type = String(formData.get("type") ?? "percent_off") as
    "percent_off" | "amount_off" | "free_months";

  const startsAt = optionalDate(formData.get("startsAt"));
  if (startsAt && "error" in startsAt)
    return fieldError({ startsAt: startsAt.error });
  ...
  const value =
    type === "amount_off"
      ...
      : parseScaled(raw, {
          scale: 1,
          min: type === "free_months" ? 0 : 1,
          max: 100,
          unit: "",
        });
  if ("error" in value) return fieldError({ value: value.error });

  const months = parseScaled(formData.get("durationPeriods"), {
    scale: 1,
    min: 1,
    max: 24,
    unit: "months",
  ...
  // B-144. 0 is the ordinary case and the column's default, so `min: 0` — an
  // empty box is "no minimum" rather than a refusal. 24 matches the cap on the
  // duration above; a minimum stay longer than two years is a lease, not a promo.
  const minStay = parseScaled(formData.get("minStayMonths"), {
    scale: 1,
    min: 0,
    max: 24,
    unit: "months",
  });
  if ("error" in minStay) return fieldError({ minStayMonths: minStay.error });
...
export async function addCodeAction(
  ...
  if (expiresAt && "error" in expiresAt)
    return fieldError({ expiresAt: expiresAt.error });

  const uses = String(formData.get("maxUses") ?? "").trim();
  const maxUses = uses ? Number(uses) : null;
  if (maxUses !== null && (!Number.isInteger(maxUses) || maxUses < 1)) {
    return fieldError({
      maxUses: "A whole number of uses, or leave it empty for unlimited.",
    });
  }
