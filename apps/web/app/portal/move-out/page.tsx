...
import { previewTenantMoveOut, tenantMoveOutLeases } from '@/lib/portal/move-out'
...
export default async function PortalMoveOutPage({
  ...
  if (!lease) {
    return (
      <div className="flex flex-col gap-4">
        ...
        <Link href="/portal/move-out" className="text-sm underline underline-offset-4">
          Choose a unit
        </Link>
      </div>
    )
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <p className="text-sm text-pretty">
          Unit {lease.unitNumber} at {lease.facilityName} is scheduled to move out on{' '}
          <strong>{formatDate(lease.pendingMoveOutDate)}</strong>. Your gate code keeps working and your
          account stays active until then. Our team will verify the unit and finish closing your
  ...
  const requestedDate = date ? new Date(`${date}T00:00:00.000Z`) : lease.minMoveOutDate
  const previewResult = await previewTenantMoveOut(actor.tenantId, lease.leaseId, requestedDate)
    ...
    <div className="flex flex-col gap-6">
      ...
      {lease.moveOutNoticeDays > 0 && (
        <p className="text-muted-foreground text-sm text-pretty">
          This unit needs at least {lease.moveOutNoticeDays} day{lease.moveOutNoticeDays === 1 ? '' : 's'}{' '}
          notice, so the earliest date you can pick is {formatDate(lease.minMoveOutDate)}.
        </p>
      )}
        ...
        <dl className="border-input flex flex-col gap-2 rounded-lg border p-4 text-sm">
          ...
          {preview.settlement.prorationCreditCents > 0 && (
            <div className="flex justify-between gap-4">
              ...
              <dd className="tabular-nums">−{formatRate(preview.settlement.prorationCreditCents)}</dd>
            </div>
          )}
          <div className="flex justify-between gap-4 border-t pt-2 font-medium">
            ...
            <dd className="tabular-nums">
              {formatRate(preview.settlement.refundDueCents || preview.settlement.amountDueCents)}
            </dd>
          </div>
        </dl>
      )}

      <AdminForm action={requestMoveOutAction} label="Request a move-out" className="flex flex-col gap-3">
        <input type="hidden" name="leaseId" value={lease.leaseId} />
        <input type="hidden" name="moveOutDate" value={isoDate(requestedDate)} />
        <p className="text-muted-foreground text-sm text-pretty">
          Your gate code and account stay active until {formatDate(requestedDate)}. Our team will verify
          the unit is empty before your account is finally closed.
        </p>
        <button
          type="submit"
