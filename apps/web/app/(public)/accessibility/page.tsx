...
// Re-verified 2026-09-08, at B-274 (the waitlist form, and the page its Spanish
// mail links to — D-122, D-130). Customer-facing, on a route and two states
// this page already lists and already scans, and the same 3.1.2 Language of
// Parts failure B-263, B-264 and B-267 fixed on the three forms beside it.
//
// **What was wrong, and it was the last form on the public site.** The
// sold-out card has been `<html lang="es">` since B-090f and the availability
// mail it produces has been Spanish since B-265; everything between them was
// English. A Spanish visitor read "Todo rentado en este momento", opened
// *Email me when a 10 foot by 20 foot is free*, was labelled *Your email*,
// pressed *Add me to the list*, and was thanked in English for joining a list
// that would then write to them in Spanish. The cancel page that mail links to
// was English too, and was listed as such in
// `ENGLISH_UNDER_A_TRANSLATED_SHELL`; its row and its `lang="en"` came out
// together, which is B-272's rule and the second time the walker's
// "lists no page that has since been translated" check has been paid.
//
// **The cancel page is the one that needed a decision rather than a
// dictionary.** Its language cannot come from `st_locale`: it is reached from
// a link in an email, opened on a phone or in a webmail tab that has never
// seen the facility page, so the cookie is absent exactly when the answer
// matters. It reads the language stored on the waitlist entry — `writingLocale`
// (D-130), the same three-step rule that chose the language of the mail — and
// declares it on `ProsePage`, because the shell above it is still deciding
// from the cookie. A Spanish entry opened in an English browser is Spanish
// prose inside `lang="en"` unless the prose says otherwise.
//
// **Two bullets were re-read and neither changes.** "Form fields have real
// labels" and the errors bullet are both about MECHANISM — B-263, B-264 and
// B-267 each recorded the same reading — and translating a label or a refusal
// neither strengthens nor weakens either. What the errors bullet claims is
// that the message is tied to its field, that what was typed survives, and
// that a refusal is announced; all three were true in both languages before
// this item and are true now.
//
// **One behaviour was deliberately NOT changed, and it is the errors bullet's
// subject, so it is named here.** The first draft routed this action through
// `keyedFieldError` like the lead form beside it — which announces "There is a
// problem with one field." and leaves the actionable sentence beside the input.
// On a NINE-field form that is right. On this one-field form it is a worse
// announcement than the one B-171 built, and `smoke.spec.ts`'s English refusal
// spec failed on it, which is how a behaviour change smuggled inside a
// translation was caught. The action resolves the key and returns the shape it
// always did; that English spec passes unchanged.
//
// **No new claim about scan coverage, no new route and no new state.** The two
// waitlist states in `SCANNED_STATES` — the form opened, the form refused — are
// the same markup with different sentences in it, and both are still scanned in
// English by `a11y.spec.ts` and `smoke.spec.ts`. `/waitlist/cancel/not-a-real-token`
// stays in `PUBLIC_SCAN_ROUTES` and renders identical English markup to a
// visitor with no cookie and no entry, which is every automated run: the loops
// carry no locale cookie, which the `/` | Spanish row in `STATE_EXCEPTIONS` has
// said since B-262. The new Spanish specs assert words rather than running axe,
// so they carry no `a11y-state:` comment.
//
// **One gap left, named rather than implied, and no row owns it.** The cancel
// page's `<title>` stays English. A translated one needs `generateMetadata`,
// which cannot read the entry without calling `cancelWaitlist` a second time —
// and that function MUTATES. A second write on every visit, to translate one
// word in a browser tab on a `noindex` page, is the wrong trade; the prose the
// reader actually reads is Spanish and declares itself.
//
// The "Where we fall short" list was re-read against this build and all three
// entries are still true and unchanged in scope: the no-JavaScript hold
// countdown (the CHECKOUT's 30-minute hold, not this form), the staff screens,
// and the embedded maps. `LAST_REVIEWED` is not bumped, per D-115 — no manual
// screen-reader pass was performed, and this item performed none.
